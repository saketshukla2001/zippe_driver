...
class _LoadingPageState extends State<LoadingPage> {
  ...
  Future<void> navigate() async {
    print('➡️ navigate() called');

    // =====================================
    // 1️⃣ DOCUMENT + APPROVAL CHECK
    // =====================================
    if (userDetails['uploaded_document'] == true &&
        userDetails['approve'] == true) {

      print('✅ Documents uploaded & approved');

      // =====================================
      // 2️⃣ SUBSCRIPTION CHECK (LAST & BLOCKING)
      // =====================================
      bool hasSubscription = await SubscriptionService.checkActive();
      print('🔐 Subscription active: $hasSubscription');

      if (!hasSubscription) {
        print('❌ No subscription → Redirecting to SubscriptionScreen');
      ...
      // =====================================
      // 3️⃣ ROLE / BIDDING BASED NAVIGATION
      // =====================================
      if (userDetails['role'] != 'owner' &&
          userDetails['enable_bidding'] == true) {

        print('🚗 Going to RidePage');
        Navigator.pushAndRemoveUntil(
          context,
    ...
    // =====================================
    // 4️⃣ DOCUMENT NOT COMPLETE
    // =====================================
    else {
      print('📄 Documents not completed → RequiredInformation');
