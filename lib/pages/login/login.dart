...
//pick image from camera
  ...
  Future<void> navigate(verify) async {
    print('➡️ navigate() called with verify = $verify');
    ...
    if (verify == true) {
      ...
      // =========================================
      // 1️⃣ DOCUMENT / APPROVAL FLOW
      // =========================================

      if (userDetails['uploaded_document'] == false) {
        print('🟡 Condition: uploaded_document = false');
        nextScreen = const RequiredInformation();
      }
      else if (userDetails['uploaded_document'] == true &&
          userDetails['approve'] == false) {
        print('🟡 Condition: uploaded_document = true BUT approve = false');
        nextScreen = const RequiredInformation();
      }
      else if (userDetails['uploaded_document'] == true &&
          userDetails['approve'] == true) {

        print('🟢 Condition: uploaded_document = true AND approve = true');

        if (userDetails['role'] != 'owner' &&
            userDetails['enable_bidding'] == true) {
          print('➡️ Navigating logic: RidePage (bidding enabled)');
          nextScreen = const RidePage();
        }
        else if (userDetails['role'] != 'owner' &&
            userDetails['enable_bidding'] == false) {
          print('➡️ Navigating logic: Maps (bidding disabled)');
          nextScreen = const Maps();
        }
        else {
          print('➡️ Navigating logic: Maps (owner)');
          nextScreen = const Maps();
        }
      }
      ...
      bool hasSubscription = await SubscriptionService.checkActive();
      print('📦 Subscription active = $hasSubscription');

      if (!hasSubscription) {
        print('❌ No active subscription → redirect to SubscriptionScreen');
        nextScreen = const SubscriptionScreen();
      } else {
        print('✅ Subscription active → continue normal flow');
      }
      ...
      if (nextScreen != null) {
        print('🚀 Final navigation to: ${nextScreen.runtimeType}');
        ...
        print('⚠️ nextScreen is NULL (no navigation)');
      }
      ...
      print('❌ verify returned error: $verify');
      setState(() {
